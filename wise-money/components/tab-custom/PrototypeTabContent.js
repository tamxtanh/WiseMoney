...
import { TransactionCreator } from "../prototype/TransactionCreator";

const PrototypeTabContent = ({ transactionList, typeApi }) => {
  // Initialize a TransactionCreator instance
  const transactionCreator = new TransactionCreator();

  useEffect(() => {
    // Assuming `TransactionCreator` maps the raw data into transaction objects
    const mappedTransactions = transactionList.map((transactionData) => {
      return transactionCreator.mapRawDataToTransaction(transactionData); // Adjust according to your creator
    });

    setTransactions(mappedTransactions);
  }, [transactionList, typeApi]);
...
export default PrototypeTabContent;
